...
class WCSPHSolver(SPHBase):
    ...
    def compute_pressure_forces_task(self, p_i, p_j, ret: ti.template()):
        ...
        if self.ps.material[p_j] == self.ps.material_fluid:
            ...
            x_j = self.ps.x[p_j]
            # Compute the pressure force contribution, Symmetric Formula
            # TODO: 该式对应 Akinci2012 中的式 9，但似乎为了对称形式多了一个dpj
            ...
            if self.ps.is_dynamic_rigid_body(p_j):
                ...
                force = -f_p * self.ps.density[p_i] * self.ps.m_V[p_i]
    ...
    def compute_non_pressure_forces_task(self, p_i, p_j, ret: ti.template()):
        ...
        if self.ps.material[p_j] == self.ps.material_fluid:
            ...
            if self.ps.is_dynamic_rigid_body(p_j):
                ...
                # aggregate to rigid force/torque (consistent with DFSPH)
                r_id = self.ps.object_id[p_j]
